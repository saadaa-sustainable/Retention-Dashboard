...
export async function POST(req: NextRequest) {
  try {
    ...
    // Looks up each row's cost_per_message directly from its first creative
    // (deterministic by ingested_at), computes ROAS = revenue / (delivered *
    // cost_per_message), and stamps it on each row pre-insert. CSV roas is
    // never touched — calculated_roas lives in its own column so the two stay
    // distinguishable.
    if (type === 'campaigns' || type === 'automations') {
      // Build a parent → cost_per_message map from the right creatives table.
      // Pick the FIRST creative (by ingested_at) when a parent has multiple
      // templates with different costs.
      ...
      if (type === 'campaigns') {
        ...
        if (cids.length) {
          const { data: cc } = await supabase
            ...
            .select('campaign_id, cost_per_message, ingested_at')
            .in('campaign_id', cids)
            .not('cost_per_message', 'is', null)
            .order('ingested_at', { ascending: true })
          for (const r of (cc || []) as { campaign_id: string; cost_per_message: string|number }[]) {
            if (!costByParent.has(r.campaign_id)) costByParent.set(r.campaign_id, Number(r.cost_per_message))
          }
        }
      } else {
        ...
        if (names.length) {
          const { data: ac } = await supabase
            ...
            .select('automation_name, cost_per_message, ingested_at')
            .in('automation_name', names)
            .not('cost_per_message', 'is', null)
            .order('ingested_at', { ascending: true })
          for (const r of (ac || []) as { automation_name: string; cost_per_message: string|number }[]) {
            if (!costByParent.has(r.automation_name)) costByParent.set(r.automation_name, Number(r.cost_per_message))
          }
        }
      }
      ...
      for (const r of rows) {
        ...
        const parent = type === 'campaigns'
          ...
          : String(r.name || '')
        const rate = costByParent.get(parent)
